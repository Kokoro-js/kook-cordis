...
class Rates extends kook_cordis_1.Service {
    ...
    constructor(ctx, config) {
        ...
        this.config = config;
        this.config.interval = this.config.interval * kook_cordis_1.Time.second;
        ...
        ctx.on('command/execute', async (command, bot, session) => {
            ...
            for (const symbol of symbols) {
                if (symbol == Rates.rateInterval)
                    interval = command[Rates.rateInterval];
                if (symbol == Rates.rateBucket)
                    [period, times] = command[Rates.rateBucket];
            }
            if (interval) {
                const key = `${exports.name}_cooldown:${session.userId}:${command.name}`;
                // 使用位图尽可能节省内存
                ctx.redis.client
                    .multi()
                    .setBit(key, 0, 1)
            ...
            if (!period)
                return;
            const bucketKey = `${exports.name}_bucket:${session.userId}:${command.name}`;
            const availableTimes = await ctx.redis.client.evalSha(this.periodExeLuaSha, {
                keys: [bucketKey],
                arguments: [String(times), String(period)],
            });
            bot
                .sendMessage(session.channelId, `指令限额 ${availableTimes}/${times + 1}，每 ${period} 秒刷新一次。`, { temp_target_id: session.userId })
                .catch(logger.error);
        });
    }
    /**
     ...
     */
    per(command, period, times) {
        times = times - 1;
        command[Rates.rateBucket] = [period, times];
        command.checkers[`bucket`] = async (bot, session) => {
            const key = `${exports.name}_bucket:${session.userId}:${command.name}`;
            const availableTimes = (await this.ctx.redis.client.get(key)) || times;
            if (availableTimes <= 0) {
                bot
                    .sendMessage(session.channelId, `你已消耗完该指令该时段的使用次数，该指令被设定为每 ${period} 秒使用 ${times} 次。`)
                    .catch(logger.error);
                return false;
            }
        };
...
(function (Rates) {
    Rates.Config = kook_cordis_1.Schema.object({
        interval: kook_cordis_1.Schema.natural().default(1),
        multi_steps: kook_cordis_1.Schema.natural().default(4),
        emit_times: kook_cordis_1.Schema.number().default(10),
    });
})(Rates || (exports.Rates = Rates = {}));
