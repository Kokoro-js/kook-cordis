...
export interface AbstactBot {
  ...
  createGuildMute(param: { guild_id: string; user_id: string; type: Kook.GuildMute.Type }): Promise<void>
  deleteGuildMute(param: { guild_id: string; user_id: string; type: Kook.GuildMute.Type }): Promise<void>
  getGuildBoostHistory(param: { guild_id: string; start_time: number; end_time: number }): Promise<Kook.List<Kook.GuildBoost>>
  ...
  createChannel(param: { guild_id: string; parent_id?: string; name: string; type?: number; limit_amount?: number; voice_quality?: string; is_category?: 0|1 }): Promise<Kook.Channel>
  updateChannel(param: { channel_id: string; name?: string; topic?: string; slow_mode?: 0|5000|10000|15000|30000|60000|120000|300000|600000|900000|1800000|3600000|7200000|21600000 }): Promise<Kook.Channel>
  ...
  createChannelRole(param: { channel_id: string; type?: 'user_id'; value?: string }): Promise<Omit<Kook.ChannelRole, 'role_id'>>
  createChannelRole(param: { channel_id: string; type: 'role_id'; value?: string }): Promise<Omit<Kook.ChannelRole, 'user_id'>>
  updateChannelRole(param: { channel_id: string; type?: 'user_id'; value?: string; allow?: number; deny?: number }): Promise<Omit<Kook.ChannelRole, 'role_id'>>
  updateChannelRole(param: { channel_id: string; type: 'role_id'; value?: string; allow?: number; deny?: number }): Promise<Omit<Kook.ChannelRole, 'user_id'>>
  deleteChannelRole(param: { channel_id: string; type?: 'role_id' | 'user_id'; value?: string }): Promise<void>

  getMessageList(param: { target_id: string; msg_id?: string; pin?: 0 | 1; flag?: 'before' | 'around' | 'after' } & Kook.Pagination): Promise<Kook.List<Kook.Message>>
  ...
  getChannelJoinedUserList(param: { guild_id: string; user_id: string } & Kook.Pagination): Promise<Kook.List<Kook.Channel>>

  getPrivateChatList(param?: Kook.Pagination): Promise<Kook.List<Omit<Kook.PrivateChat, 'is_friend' | 'is_blocked' | 'is_target_blocked'>>>
  ...
  getDirectMessageList(param: { target_id?: string; chat_code?: string; msg_id?: string; flag?: 'before' | 'around' | 'after' } & Kook.Pagination): Promise<{ items: Kook.Message[] }>
  createDirectMessage(param: { target_id?: string; chat_code?: string; type?: Kook.MessageType; content: string; quote?: string; nonce?: string}): Promise<Kook.MessageReturn>
  ...
  getToken(param: { grant_type: 'authorization_code'; client_id: string; client_secret: string; code: string; redirect_uri: string}): Promise<Kook.AccessToken>
  ...
  getGuildRoleList(param: { guild_id: string } & Kook.Pagination): Promise<Kook.List<Kook.GuildRole>>
  createGuildRole(param: { name?: string; guild_id: string }): Promise<Kook.GuildRole>
  updateGuildRole(param: { guild_id: string; role_id: number } & Partial<Omit<Kook.GuildRole, 'role_id'>>): Promise<Kook.GuildRole>
  deleteGuildRole(param: { guild_id: string; role_id: number }): Promise<void>
  grantGuildRole(param: { guild_id: string; user_id?: string; role_id: number }): Promise<Kook.GuildRoleReturn>
  revokeGuildRole(param: { guild_id: string; user_id?: string; role_id: number }): Promise<Kook.GuildRoleReturn>
  ...
  getInviteList(param: { guild_id?: string; channel_id?: string } & Kook.Pagination): Promise<Kook.List<Kook.Invite>>
